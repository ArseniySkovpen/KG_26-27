...
bool ObjLoader::LoadMtl(const std::string& mtlPath, std::vector<Material>& mats)
{
    ...
    while (std::getline(f, line))
    {
        ...
        else if (curIdx >= 0)
        {
            ...
            {
                // „итаем токены Ч пропускаем параметры вида -bm 0.45, -s 1 1 1 и т.д.
                // »м€ файла Ч последний токен который не начинаетс€ с '-'
                std::string tok, tex;
                while (ss >> tok)
                {
                    if (tok[0] == '-')
                    {
                        // Ёто параметр Ч пропускаем его и следующее значение
                        std::string val;
                        ss >> val;
                    }
                    else
